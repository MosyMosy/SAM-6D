...
def get_parser():
    ...
    parser.add_argument(
        "--model", type=str, default="pose_estimation_model", help="name of model"
    )
    parser.add_argument(
    ...
    parser.add_argument("--iter", type=int, default=0, help="iter num. for testing")
    parser.add_argument("--view", type=int, default=42, help="view number of templates")
    # parser.add_argument("--exp_id",
    #                     default=f"fastsam_{time.strftime('%Y%m%d-%H:%M')}",
    #                     help="experiment id")
    parser.add_argument(
...
def init():
    args = get_parser()
    exp_name = osp.splitext(args.config.split("/")[-1])[0] + "_id_" + str(args.det_name)
    exp_name += "_all" if args.obj_id is None else f"obj-{str(args.obj_id)}"
    exp_name += f"_{time.strftime('%Y%m%d-%H-%M')}"
    ...
    with tqdm(total=len(dataloder)) as t:
        for i, data in enumerate(dataloder):
            ...
            for j in range(n_batch):
                ...
                inputs["rgb_choose"] = data["rgb_choose"][0][
                    start_idx:end_idx
                ].contiguous()
            ...
            t.set_description("Test [{}/{}]".format(i + 1, len(dataloder)))
            t.update(1)
...
if __name__ == "__main__":
    ...
    if cfg.checkpoint_path == "none":
        checkpoint = os.path.join(
            cfg.log_dir, "checkpoint_iter" + str(cfg.test_iter).zfill(6) + ".pth"
        )
    else:
    ...
    if cfg.dataset == "all":
        ...
        for dataset_name in datasets:
            ...
            save_path = os.path.join(
                cfg.log_dir, dataset_name + "_eval_iter" + str(cfg.test_iter).zfill(6)
            )
            ...
            test(
                ...
                save_path,
                dataset_name,
                detetion_paths[dataset_name].format(cfg.det_name),
            )
